...
Pass `onSelect` to the machine's context to perform some custom logic when an
item is selected. When an item is selected, the machine invokes the `onSelect`
callback. This callback is invoked with the `value` of the item.
